...
const FS=[
  ...
  {id:701,cat:"Spa",sub:"Moroccan Bath",name:"ሞሮኮ ባዝ በሞሮኮ አፈር እና የሞሮኮ ስኳርፕ",price:2800,cm:10,es:"Spa",bk:true,dm:120},
  ...
  {id:710,cat:"Spa",sub:"Moroccan Bath",name:"አምባር ስፔሻል - ቀረፋ ፣ አቢሽ ፣ ቡና ፣ ማር ፣ አርጎ ፣ ሰሊጥ ፣ ፍራፍሬ + የጀሮ ሕክምነት + 30min ማሳጅ + የጸጉር ስራ",price:5000,cm:10,es:"Spa",bk:true,dm:180},
  ...
  {id:906,cat:"Spa",sub:"Massage",name:"Deep Tissue with Shea Butter 60min",price:3000,cm:10,es:"Spa",bk:true,dm:60},
...
// Different chimes for different events
function chime(type="info"){
  try{
    const ctx=new(window.AudioContext||window.webkitAudioContext)();
    const patterns={
      info:   [{f:523,t:0},{f:659,t:0.15},{f:784,t:0.3}],
      booking:[{f:784,t:0},{f:988,t:0.15},{f:1047,t:0.3},{f:1319,t:0.45}],
      payment:[{f:392,t:0},{f:523,t:0.2},{f:659,t:0.4}],
      success:[{f:659,t:0},{f:784,t:0.15},{f:1047,t:0.3}],
      warning:[{f:440,t:0},{f:440,t:0.2}],
    };
    (patterns[type]||patterns.info).forEach(({f,t})=>{
...
function checkConflict(bks,form,svcs){
  ...
  if(svc.sub==="Moroccan Bath"){const tot=overlap.filter(b=>b.serviceCategory==="Spa").reduce((s,b)=>s+b.people,0)+Number(form.people||1);if(tot>4)return "⚠️ Morocco Bath room may be over capacity (max 4 people).";}
  if(svc.sub==="Steam & Sauna"&&overlap.filter(b=>b.serviceName&&b.serviceName.includes("Sauna")).length>0)return "⚠️ Steam & Sauna has overlapping bookings. Check availability.";
...
function EthPicker({value,onChange,label}){
  ...
  function pick(y,m,d){const g=ethToGreg(y,m,d);onChange(g);setShow(false);}
...
export default function App(){
  ...
  // FIXED: coQ now filters today only
  ...
  // FIXED: serviceId always string, compared with Number() when needed
  ...
  function unlockPin(){const f=staff.find(s=>s.id===user.id&&s.password===pinInput);if(f){setPinLocked(false);setPinInput("");setPinErr("");}else setPinErr("Wrong password. Try again.");}
  ...
  // Auto-clear stale open visits at end of day
  useEffect(()=>{
    const t=setInterval(async()=>{
      const now=new Date();
      if(now.getHours()>=CLOSE_HOUR){
        const stale=visits.filter(v=>v.date===todayStr()&&!["Paid & Closed","Cancelled"].includes(v.status));
        for(const v of stale){await supabase.from("visits").update({status:"Cancelled",note:(v.note?v.note+" ":"")+"[Auto-closed end of day]"}).eq("id",v.id);}
      }
    },60000);
  ...
  // FIXED REALTIME: broadcasts changes instantly to all devices
  useEffect(()=>{
    if(!user)return;
    const role=user.role;
    const uid=user.id+"_"+Math.random().toString(36).slice(2);

    const vs=supabase.channel("visits_"+uid,{config:{broadcast:{self:false}}})
      ...
      .on("postgres_changes",{event:"UPDATE",schema:"public",table:"visits"},p=>{
        ...
        if(role===ROLES.SUPERVISOR&&p.new.status==="In Service")push("🔄 Service added for "+p.new.name,"info");
      })
      .on("postgres_changes",{event:"DELETE",schema:"public",table:"visits"},p=>{setVisits(prev=>prev.filter(x=>x.id!==p.old.id));})
      .subscribe();

    const bs=supabase.channel("bookings_"+uid,{config:{broadcast:{self:false}}})
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"bookings"},p=>{
        ...
        if(role===ROLES.SUPERVISOR||role===ROLES.MANAGER)push("📅 New booking: "+p.new.customer_name+" at "+p.new.time,"booking");
        if(role===ROLES.RECEPTION)push("📅 Booking added: "+p.new.customer_name,"booking");
      })
      .on("postgres_changes",{event:"UPDATE",schema:"public",table:"bookings"},p=>{const b=dbBk(p.new);setBks(prev=>prev.map(x=>x.id===b.id?b:x));})
      .on("postgres_changes",{event:"DELETE",schema:"public",table:"bookings"},p=>{setBks(prev=>prev.filter(x=>x.id!==p.old.id));})
      .subscribe();

    const es=supabase.channel("exps_"+uid,{config:{broadcast:{self:false}}})
    ...
    // Polling fallback every 8 seconds to ensure sync
  ...
  // FIXED: checkout only shows TODAY's visits
  const coList=useMemo(()=>{
    const q=coQ.toLowerCase().trim();
    const todayVisits=visits.filter(v=>v.date===todayStr());
    if(!q)return todayVisits.filter(v=>!["Paid & Closed","Cancelled"].includes(v.status));
    return todayVisits.filter(v=>String(v.queue).includes(q)||v.name.toLowerCase().includes(q)||v.phone.includes(q));
  },[visits,coQ]);
  ...
  // FIXED: only bookable spa services in booking form
  ...
  function recall(){const f=custs.find(c=>c.phone===rPhone.trim());if(f){setRName(f.name);setRmsg("✓ "+f.name+" — "+f.totalVisits+" previous visits");}else setRmsg("New customer — not in system yet");}
  async function register(){
    ...
    if(error){alert("Error saving. Check internet connection.");setSaving(false);return;}
  ...
  async function saveBk(){
    // FIXED: proper Number comparison for serviceId
    const sid=Number(bkF.serviceId);
    const s=svcs.find(sv=>sv.id===sid);
    if(!bkF.customerName.trim()||!bkF.customerPhone.trim()||!sid||!s||!bkF.date||!bkF.time)return alert("Fill all required fields including service.");
  ...
  if(user&&pinLocked)return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#0f1720,#1d2a36)"}}>
      <div style={{background:"#fff",borderRadius:24,padding:40,width:"100%",maxWidth:340,margin:"0 16px",boxShadow:"0 20px 60px rgba(0,0,0,0.4)",textAlign:"center"}}>
        <div style={{fontSize:44,marginBottom:8}}>🔒</div>
        <h2 style={{margin:"0 0 4px"}}>Session Locked</h2>
        <p style={{color:"#6b7280",fontSize:13,marginBottom:20}}>Enter password to continue as {user.name}</p>
  ...
  if(!user)return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#0f1720,#1d2a36)"}}>
      <div style={{background:"#fff",borderRadius:24,padding:40,width:"100%",maxWidth:380,margin:"0 16px",boxShadow:"0 20px 60px rgba(0,0,0,0.4)"}}>
  ...
  return(<div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f1720 0%,#1d2a36 38%,#f6efe3 38%,#fffaf2 100%)",fontFamily:"Segoe UI,Arial,sans-serif",color:"#111827"}}>
    ...
    <div style={{maxWidth:1400,margin:"0 auto",padding:sc.mob?"12px":"28px"}}>
      <header style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",color:"white",marginBottom:14,flexWrap:"wrap",gap:8}}>
        <div>
          <p style={{color:"#e0b85a",fontWeight:900,letterSpacing:2,margin:"0 0 2px",fontSize:11}}>AMBAR SPA & BEAUTY</p>
      ...
      {sc.mob?(
        <div style={{marginBottom:10}}>
          ...
          {mobNav&&<div style={{background:"#fff",borderRadius:14,padding:10,marginTop:6,border:"1px solid #e6c977"}}>{allTabs.map(t=><button key={t} style={{...tab===t?S.tabA:S.tab,display:"block",width:"100%",marginBottom:4,textAlign:"left"}} onClick={()=>{setTab(t);setMobNav(false);}}>{t}</button>)}</div>}
        </div>
      ):(
        <>
      ...
      {tab==="Reception"&&<main style={{display:"grid",gridTemplateColumns:gc,gap:14}}>
        ...
        <section style={S.card}>
          <h2 style={S.ct}>Today's Queue</h2>
          <p style={S.hlp}>{new Date().toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long"})}</p>
          {todayV.length===0&&<EMP>No customers registered yet today.</EMP>}
          {todayV.map(v=><div key={v.id} style={S.li}>
            <div>
              <b style={{fontSize:15}}>#{v.queue} — {v.name}</b>
              {v.groupName&&<p style={S.hlp}>{v.groupName}</p>}
              {v.note&&<p style={S.hlp}>📝 {v.note}</p>}
            </div>
      ...
      {tab==="Checkout"&&<main style={{display:"grid",gridTemplateColumns:gc,gap:14}}>
        <section style={S.card}>
          <h2 style={S.ct}>Checkout — Today</h2>
        ...
        <section style={S.card}>
          {!act?<EMP>← Select a customer to process payment.</EMP>
           ...
           :<>
            ...
            <p style={S.hlp}>Tips go directly to employees, not counted as revenue.</p>
            <div style={S.r2}>
              ...
              <input style={S.inp} type="number" value={tipAmt} onChange={e=>setTipAmt(e.target.value)} placeholder="Amount (Birr)"/>
            </div>
        ...
        {showBkF&&user.role!=="supervisor"&&<div style={{background:"#fffaf2",border:"1px solid #e0b85a",borderRadius:16,padding:18,marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:sc.mob?"1fr":"1fr 1fr",gap:10}}>
            ...
            <div style={{gridColumn:"1/-1"}}>
              <L>Service *</L>
              <select style={{...S.inp,background:bkF.serviceId?"#fffdf7":"#fff9f0"}} value={bkF.serviceId===0?"":String(bkF.serviceId||"")} onChange={e=>{const sid=e.target.value;setBkF(p=>({...p,serviceId:sid}));if(sid){const warn=checkConflict(bks,{...bkF,serviceId:sid},svcs);setBkWarn(warn||"");}else setBkWarn("");}}>
      ...
      {tab==="Service Setup"&&<section style={S.card}>
        ...
        <div style={{display:"grid",gridTemplateColumns:sc.mob?"1fr":"1fr 1fr",gap:20,marginBottom:16}}>
          ...
          <div>
            <h3 style={S.sh}>Add New Service</h3>
        ...
        <HR/>
        <h3 style={S.sh}>All Services ({svcs.length}) — grouped by category</h3>
        ...
        {(svcF==="All"?cats:[svcF]).map(cat=>{
          ...
          return <div key={cat} style={{marginBottom:20}}>
            <h4 style={{margin:"0 0 8px",fontSize:14,fontWeight:900,color:"#6b4c11",borderBottom:"2px solid #e0b85a",paddingBottom:6}}>{cat} ({catSvcs.length} services)</h4>
            {subs.map(sub=><div key={sub} style={{marginBottom:12}}>
              {sub&&<p style={{...S.hlp,fontWeight:800,margin:"0 0 4px"}}>{sub}</p>}
              <div style={{overflowX:"auto"}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 80px 55px 65px 50px 32px",gap:4,padding:"4px 8px",background:"#f5e7c0",borderRadius:8,marginBottom:4,fontSize:10,fontWeight:800,color:"#6b4c11",minWidth:480}}>
                  <span>Name</span><span>Price</span><span>Cm%</span><span>Min</span><span>Book</span><span></span>
                </div>
                {catSvcs.filter(s=>s.sub===sub).map(s=><div key={s.id} style={{display:"grid",gridTemplateColumns:"1fr 80px 55px 65px 50px 32px",gap:4,padding:"4px 8px",background:"#fffaf2",borderRadius:8,marginBottom:3,alignItems:"center",border:"1px solid #ecdba3",minWidth:480}}>
      ...
      {tab==="Customers"&&<section style={S.card}>
        <h2 style={S.ct}>Customer Database ({custs.length})</h2>
        ...
        {fCusts.map(c=>{
          ...
          return <div key={c.id} style={S.li}>
            <div><b style={{fontSize:15}}>{c.name}</b><p style={S.hlp}>{c.phone} · {c.id}</p><p style={S.hlp}>Favourite: {fav} · {cbks.length} bookings · {cv.length} visits</p></div>
      ...
      {tab==="Payroll"&&<section style={S.card}>
        ...
        {emps.filter(e=>showFired||e.active).map(emp=>{
          ...
          const d=Number(emp.salary||0)/30;const ad=d*Number(emp.absentDays||0);
          const net=Number(emp.salary||0)+Number(extra?.commissionTotal||0)-Number(emp.loan||0)-Number(emp.brokerFee||0)-Number(emp.otherDeduction||0)-ad;
      ...
      {tab==="Staff"&&<section style={S.card}>
        ...
        <div style={{background:"#fffaf2",border:"1px solid #e0b85a",borderRadius:14,padding:16,marginBottom:16}}>
          <h3 style={{margin:"0 0 14px",fontWeight:800,fontSize:15}}>{editStaff?"Edit Account: "+editStaff.id:"Add / Update Staff Account"}</h3>
          <div style={{display:"grid",gridTemplateColumns:sc.mob?"1fr":"1fr 1fr 1fr 1fr",gap:10,marginBottom:12}}>
            <div><L>Username (login ID)</L><input style={{...S.inp,background:editStaff?"#f3f4f6":"#fffdf7"}} value={nStaff.id} onChange={e=>setNStaff(p=>({...p,id:e.target.value}))} placeholder="e.g. reception1" disabled={!!editStaff}/></div>
            ...
            <div><L>{editStaff?"New Password":"Password"}</L><input style={S.inp} type="password" value={nStaff.password} onChange={e=>setNStaff(p=>({...p,password:e.target.value}))} placeholder={editStaff?"Leave blank to keep current":"Password"}/></div>
          </div>
          <div style={S.r2}>
            <button style={S.btnP} onClick={saveStaff}>{editStaff?"Update Account":"Save Account"}</button>
            {editStaff&&<button style={S.btnS} onClick={()=>{setEditStaff(null);setNStaff({id:"",name:"",role:"reception",password:""});}}>Cancel</button>}
          </div>
        </div>
        <h3 style={S.sh}>All Staff Accounts ({staff.length})</h3>
        {staff.map(s=><div key={s.id} style={{...S.li,flexWrap:"wrap",gap:10,opacity:s.active?1:0.6}}>
          <div>
            <b style={{fontSize:15}}>{s.name}</b>
            <span style={{background:s.role==="manager"?"#334155":s.role==="supervisor"?"#1e40af":"#f5e7c0",color:s.role==="manager"?"#e0b85a":s.role==="supervisor"?"#fff":"#6b4c11",borderRadius:14,padding:"2px 10px",fontSize:11,fontWeight:700,marginLeft:8}}>{s.role}</span>
    ...
    {visit.services.map(line=>{
      ...
      return <div key={line.lineId} style={{background:done?"#f8fafb":"#fffaf2",border:"1px solid "+(done?"#d1d5db":"#ecdba3"),borderRadius:12,padding:10,marginBottom:7}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end"}}>
          <div><p style={{fontSize:10,fontWeight:700,color:"#6b4c11",margin:"0 0 2px"}}>Qty</p>
            <input style={{width:55,padding:"6px 8px",borderRadius:8,border:"1px solid #c7b06a",background:"#fff",fontSize:12}} type="number" min="1" value={line.qty} onChange={e=>onUpd(line.lineId,"qty",e.target.value)} disabled={locked}/></div>
          {!isSv&&<>
            <div><p style={{fontSize:10,fontWeight:700,color:"#6b4c11",margin:"0 0 2px"}}>Discount</p>
              <input style={{width:80,padding:"6px 8px",borderRadius:8,border:"1px solid #c7b06a",background:"#fff",fontSize:12}} type="number" value={line.discount} onChange={e=>onUpd(line.lineId,"discount",e.target.value)} disabled={locked}/></div>
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
              <p style={{fontSize:10,fontWeight:700,color:"#6b4c11",margin:0}}>Free</p>
              <input type="checkbox" checked={line.free} onChange={e=>onUpd(line.lineId,"free",e.target.checked)} disabled={locked} style={{width:16,height:16}}/></div>
          </>}
          <div><p style={{fontSize:10,fontWeight:700,color:"#6b4c11",margin:"0 0 2px"}}>Preferred</p>
          ...
          <div><p style={{fontSize:10,fontWeight:700,color:"#6b4c11",margin:"0 0 2px"}}>Who Did It?</p>
          ...
          <div><p style={{fontSize:10,fontWeight:700,color:"#6b4c11",margin:"0 0 2px"}}>Status</p>
            <select style={{padding:"6px 8px",borderRadius:8,border:"1px solid #c7b06a",background:done?"#f0fdf4":"#fff",fontSize:12}} value={line.status} onChange={e=>onUpd(line.lineId,"status",e.target.value)} disabled={locked}>
...
function PS({emps,empC,period}){return <div style={{fontFamily:"Arial,sans-serif",padding:32}}>
  ...
  <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
    <thead><tr style={{background:"#e0b85a"}}>{["Employee","Section","Base","Commission","Absent Ded.","Loan","Broker","Other","NET PAY"].map(h=><th key={h} style={{border:"1px solid #999",padding:"7px 9px",textAlign:"left"}}>{h}</th>)}</tr></thead>
